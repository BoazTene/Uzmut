class Player{
    ...
    //sx=(this.that.x-200)*this.that.scale, sy=(this.that.y-150)-(canvas.height-15000), sw=(this.that.width+400)*this.that.scale, sh=this.that.height+300, dx=this.that.x-200, dy=this.that.y-150, dw=this.that.width+400, dh=this.that.height+300
    async clear(sx=(this.x-200)*this.scale, sy=(this.y-150)-(canvas.height-15000), sw=(this.width+400)*this.scale, sh=this.height+300, dx=this.x-200, dy=this.y-150, dw=this.width+400, dh=this.height+300){
        return new Promise(async resolve => {
            var clear = new Clear()
...
class Move{
    ...
    //sx=this.that.x, sy=this.that.y-(canvas.height-15000), sw=this.that.width, sh=this.that.height, dx=this.that.x, dy=this.that.y, dw=this.that.width, dh=this.that.height
    async clear(sx=(this.that.x-200)*this.that.scale, sy=(this.that.y-150)-(canvas.height-15000), sw=(this.that.width+400)*this.that.scale, sh=this.that.height+300, dx=this.that.x-200, dy=this.that.y-150, dw=this.that.width+400, dh=this.that.height+300){
        return new Promise(async resolve => {
            var clear = new Clear()
            await clear.replace("background_test.jpg", sx, sy, sw, sh, dx, dy, dw, dh);
    ...
    async moveToRight(){
        ...
        // await this.clear(that.x, that.y-(canvas.height-15000), that.width, that.height, that.x, that.y, that.width, that.height);
